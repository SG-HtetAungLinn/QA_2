$(document).ready(function () {
    ...
    form.on("submit", function (e) {
        ...
        if (checkBannedWords(formData.title)) {
            titleError
                .text(`The word "${checkBannedWords(formData.title)}" is not allowed.`)
                .show();
            titleInput.addClass("is-invalid");
            showAlert(`The word "${checkBannedWords(formData.title)}" is not allowed in the title.`, "danger");
            return;
        }
        if (checkBannedWords(formData.question)) {
            questionError
                .text(`The word "${checkBannedWords(formData.question)}" is not allowed.`)
                .show();
